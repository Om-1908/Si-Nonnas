  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl">
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => { setFilter(''); fetch(); }} className={`px-4 py-2 rounded-sm text-sm ${!filter ? 'bg-primary-container text-on-primary-container' : 'bg-surface-container-high text-muted'}`}>All</button>
        {[5, 4, 3, 2, 1].map(n => (
          <button key={n} onClick={() => { setFilter(n); fetch(n); }}
          ...
          <div key={r._id} className="card">
            ...
            {r.reply ? (
              ...
              <div className="mt-2 animate-fade-in">
                <textarea value={replyText} onChange={e => setReplyText(e.target.value)} className="input-field h-16 resize-none mb-2" placeholder="Write a reply..." />
              ...
              <button onClick={() => { setReplyId(r._id); setReplyText(''); }} className="text-primary text-xs hover:underline mt-1">Reply</button>
            )}
          </div>
        ))}
      </div>
